...
class ChapterAction extends GlobalAction {
    ...
    public function index($book) {
        //书籍信息
        $isbook = M('Book')->where(array('book_id' => $book))->field('book_id,cp_id,fu_web,book_name')->find();
    ...
    public function add($book) {
        if ($this->isPost()) {
            ...
            $arr['content'] =  mb_convert_encoding ($_POST['content'],'UTF-8',"auto");
            //$content['content'] = mb_convert_encoding($content['content'], "UTF-8");
            //是否有留言
    ...
    public function look($book, $num) {
            ...
            $content['content'] = str_replace("\r\n\r\n", "\n", str_replace(" ", "&nbsp;", $contents['content'])); //内容部分处理
            $content['content'] = str_replace("\n\n", "\n", $content['content']);
            $content['content'] = str_replace("\n", "<br>", $content['content']);
            //$content['content'] = preg_replace("<(\S*?)[^>]*>.*?|<.*? />","<br>",$content['content']);
    ...
    //批量更新章节时间
    public function addtimes($book){
       
        //print_r($_GET[book]);//exit;
         if($this->isPost()){
            //print_r($_POST);
            $time = $_POST[attribute];
            //$bookid = $_POST[bookid];
            // echo $time;
            // echo $bookid;
            $content = M('BookContent')->where(array('fu_book' => $book))->field('fu_book,num,attribute')->select();
            //print_r($content);
            foreach ($content as $key => $value) {
                $sec = strtotime($time) + 86400 * $value[num];
                ...
                $arr['fu_book'] = $book;
                //print_r($arr);
                $isok = M('BookContent')->where(array("fu_book"=>$book,"num"=>$value[num]))->save($arr);
                if($isok){
                   // echo "success";
                }else{
                    echo "fail";
