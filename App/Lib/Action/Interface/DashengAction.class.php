<?php
class DashengAction extends Action {

    private $cp_id = 56;
    private $cp_name='大圣中文网';
    private $key = 'd4caa707f05817c4afe24e82e867b985';
    private $secret='740aba8c5145827b6ed94e011d6f6b39';
    ...
    public function index($cp) {
        ...
        $this->display('Interface@Dasheng');
    }
    ...
    public function  test(){
      header('Content-type: text/html;charset=utf-8');
       ...
       public function  booklistapi(){
        header("Content-type: text/html; charset=utf-8");
        $token = md5(md5($this->key."getbooklist").$this->secret);
        $booklists = file_get_contents("https://api.dashengzw.com/channel/getbooklist?key=".$this->key."&token=".$token);
      ...
      public function  getbookinfo($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $token = md5(md5($this->key."getbookinfo".$partnerbookid).$this->secret);
        $bookinfo = file_get_contents("https://api.dashengzw.com/channel/getbookinfo?bookid=".$partnerbookid."&key=".$this->key."&token=".$token);
      ...
      private  function  getchapters($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $token = md5(md5($this->key."getbookchapterlist".$partnerbookid).$this->secret);
        $chapters = file_get_contents("https://api.dashengzw.com/channel/getbookchapterlist?bookid=".$partnerbookid."&key=".$this->key."&token=".$token);
       ...
       private  function  getcontent($partnerbookid,$partnerchapterid){
            header("Content-type: text/html; charset=utf-8");
            $token = md5(md5($this->key."getbookchapter".$partnerbookid.$partnerchapterid).$this->secret);
            $content =file_get_contents("https://api.dashengzw.com/channel/getbookchapter?bookid=".$partnerbookid."&chapterid=".$partnerchapterid."&key=".$this->key."&token=".$token);
      ...
      public function booklist(){
          ...
          foreach ($booklist as $key => $value) {
            if($key<100){
              ...
              $bookpic=(string)$bookinfo->bookimg;//需要修改
              $gender=(int)$bookinfo->category==1?1:2;         //1男2女                  //需要修改
              $state=(int)$bookinfo->endstatus;   //1 连载 2完本
              $keywords=(string)$bookinfo->tags;    //需要修改
              $description=(string)$bookinfo->bookintro;//需要修改
              $gt = (int)$bookinfo->classid;
