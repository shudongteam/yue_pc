<?php
class GuijiejieAction extends Action {
    ...
    public function index($cp) {
        ...
        $this->display('Interface@Guijiejie');
    }
    ...
    public function  test(){
       ...
       public function  booklistapi(){
        ...
        $booklists = file_get_contents("https://www.guijj.com/api/exp/yueming.php?sign=fb1f41e419f4acd9ce6bd713ff07dccb&act=list");
      ...
      public function  getbookinfo($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $bookinfo = file_get_contents("https://www.guijj.com/api/exp/yueming.php?sign=fb1f41e419f4acd9ce6bd713ff07dccb&act=info&book_id=".$partnerbookid);
      ...
      private  function  getchapters($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $chapters = file_get_contents("https://www.guijj.com/api/exp/yueming.php?sign=fb1f41e419f4acd9ce6bd713ff07dccb&act=chapters&book_id=".$partnerbookid);
       ...
       private  function  getcontent($partner_bookid,$partner_chapterid){
            header("Content-type: text/html; charset=utf-8");
            $content = file_get_contents("https://www.guijj.com/api/exp/yueming.php?sign=fb1f41e419f4acd9ce6bd713ff07dccb&act=content&book_id=".$partner_bookid."&chapter_id=".$partner_chapterid);
            $content=preg_replace("#\\\u([0-9a-f]{4})#ie", "iconv('UCS-2BE', 'UTF-8', pack('H4', '\\1'))", $content);
            //$content = preg_replace('/s+/', '',$content); 
            $contents = $this->ext_json_decode($content);

            //return $contents;
            return   $contents->data->content;
      }
      ...
      public function booklist(){
          ...
          foreach ($booklist as $key => $value) {
            if($key<100){
              ...
              $bookname=(string)$bookinfo->data->name;   //需要修改
              $authorname=(string)$bookinfo->data->author;  //需要修改
              $bookpic=(string)$bookinfo->data->cover;//需要修改
              $gender=2;         //1男2女                  //需要修改
              $state=(int)$bookinfo->data->status==0?1:2;;   //1 连载 2完本
              $keywords=(string)$bookinfo->data->keywords;    //需要修改
              $description=(string)$bookinfo->data->abstract;//需要修改
              $type = (int)$bookinfo->data->category;
              // echo $bookname;
              // echo "<br>";
              // echo $authorname;
              // echo "<br>";
