<?php
class LuochenAction extends Action {

    private $cp_id = 60;
    private $cp_name='落尘文学网';
    ...
    public function index($cp) {
        ...
        $this->display('Interface@Luochen');
    }
    ...
    public function  test(){
      ...
      //   $chapterid = 808213;
       ...
       public function  booklistapi(){
        header("Content-type: text/html; charset=utf-8");
        $booklists = file_get_contents("http://cp.api.luochen.com/interface/YueMing/bookList");
      ...
      public function  getbookinfo($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $bookinfo = file_get_contents("http://cp.api.luochen.com/interface/YueMing/bookInfo?bookid=".$partnerbookid);
      ...
      private  function  getchapters($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $chapters = file_get_contents("http://cp.api.luochen.com/interface/YueMing/chapters?bookid=".$partnerbookid);
       ...
       private  function  getcontent($partnerbookid,$partnerchapterid){
            header("Content-type: text/html; charset=utf-8");
            $content =file_get_contents("http://cp.api.luochen.com/interface/YueMing/chapterContent?bookid=".$partnerbookid."&chapterid=".$partnerchapterid);
      ...
      public function booklist(){
          ...
          foreach ($booklist as $key => $value) {
            if($key<100){
              ...
              $gender=(int)$bookinfo->gender;         //1男2女                  //需要修改
              $state=(int)$bookinfo->writestatus;   //1 连载 2完本
              $keywords=(string)$bookinfo->keywords;    //需要修改
              $description=(string)$bookinfo->zzjs;//需要修改
              $type = (int)$bookinfo->cname;
              // echo $bookname;
              // echo "<br>";
              // echo $authorname;
              // echo "<br>";
