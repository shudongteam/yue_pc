<?php
class HuayuAction extends Action {

    private $cp_id = 25;
    private $cp_name='华语思阅';
    ...
    public function  test(){
      ...
      //   print_r($this->getcontent($bookid,$chapterid));
       ...
       public function  booklistapi(){
        header("Content-type: text/html; charset=utf-8");
         $booklists = file_get_contents("http://www.9d9h.cn/api_yueming.php?api_id=1935008768&api_secret_key=rL8LI8js2h7xj8pf90lR");
      ...
      public function  getbookinfo($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $bookinfo = file_get_contents("http://www.9d9h.cn/api_yueming.php?api_id=1935007338&api_secret_key=rL8Loc9oigg1j8pf90lR&articleid=".$partnerbookid);
      ...
      private  function  getchapters($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $chapters = file_get_contents("http://www.9d9h.cn/api_yueming.php?api_id=1935000527&api_secret_key=rL8Loc9oa83jux4d90lR&articleid=".$partnerbookid);
       ...
       private  function  getcontent($partner_bookid,$partner_chapterid){
            header("Content-type: text/html; charset=utf-8");
            $content = file_get_contents("http://www.9d9h.cn/api_yueming.php?api_id=1935927527&api_secret_key=rL8LoY7e383jux4d90lR&articleid=".$partner_bookid."&chapterid=".$partner_chapterid);
      ...
      public function booklist(){
          ...
          foreach ($booklist as $key => $value) {
            if($key<100){
              ...
              $bookname=(string)$bookinfo[0]->bookname;   //需要修改
              $authorname=(string)$bookinfo[0]->authorname;  //需要修改
              $bookpic=(string)$bookinfo[0]->bookpic;//需要修改
              $gender=(int)$bookinfo[0]->gender;         //1男2女                  //需要修改
              $state=(int)$bookinfo[0]->writestatus==1?1:2;;   //1 连载 2完本
              $keywords=(string)$bookinfo[0]->keywords;    //需要修改
              $description=(string)$bookinfo[0]->zzjs;//需要修改
              $type = (int)$bookinfo[0]->class;
              // echo $bookname;
              // echo "<br>";
              // echo $authorname;
              // echo "<br>";
