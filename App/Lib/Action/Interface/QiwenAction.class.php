<?php
class QiwenAction extends Action {

    private $cp_id = 57;
    private $cp_name='奇文文化';
    private $key = 'yueming';
    private $sign='2a79cbbc49fba4cd71f432bc2ca6e932';
    ...
    public function  test(){
      header('Content-type: text/html;charset=utf-8');
       ...
       public function  booklistapi(){
        header("Content-type: text/html; charset=utf-8");
        $booklists = file_get_contents("http://47.96.181.151:8002/Out/booklist/?sign=".$this->sign."&key=".$this->key);
      ...
      public function  getbookinfo($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $bookinfo = file_get_contents("http://47.96.181.151:8002/Out/bookinfo/?bookid=".$partnerbookid."&sign=".$this->sign."&key=".$this->key);
      ...
      private  function  getchapters($partnerbookid){
        header("Content-type: text/html; charset=utf-8");
        $chapters = file_get_contents("http://47.96.181.151:8002/Out/chapter/?bookid=".$partnerbookid."&sign=".$this->sign."&key=".$this->key);
       ...
       private  function  getcontent($partnerbookid,$partnerchapterid){
            header("Content-type: text/html; charset=utf-8");
            $content =file_get_contents("http://47.96.181.151:8002/Out/content/?bookid=".$partnerbookid."&chapterid=".$partnerchapterid."&sign=".$this->sign."&key=".$this->key);
      ...
      public function booklist(){
          ...
          foreach ($booklist as $key => $value) {
            if($key<100){
              ...
              $gender=(int)$bookinfo->channel==0?1:2;         //1男2女                  //需要修改
              $state=(int)$bookinfo->status==0?1:2;   //1 连载 2完本
              $keywords="";    //需要修改
              $description=(string)$bookinfo->intro;//需要修改
              $gt = (int)$bookinfo->cateid;
